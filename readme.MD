...
* （A）用户打开客户端以后，客户端要求用户给予授权(注：这里想像一个场景，用户在客户端中点击"访问云相册"按钮，于是客户端要求用户授权)。

* （B）用户同意给予客户端授权。

* （C）客户端使用上一步获得的授权，向认证服务器申请令牌（注：根据作者的名词定义，这里的客户端即第三方应用程序，因为相对认证服务器而言，第三方应用程序是客户端，
不要理解为浏览器。我们假想这一步认证服务器返回给浏览器一个认证表单）。

* （D）认证服务器对客户端进行认证以后，确认无误，同意发放令牌(注:假设这一步是提交上一步认证服务器响应的认证表单,同时想一想，认证需要用户名密码这些凭证吗？
第三方应用程序怎么保存这个返回的令牌？)。

* （E）客户端使用令牌，向资源服务器申请获取资源(注:这里的场景是客户端访问用户的云相册，这是第一步用户的意图)。

* （F）资源服务器确认令牌无误，同意向客户端开放资源（注：简单点考虑，资源服务器和认证服务器是同一台服务器。服务器需要令牌，所以请求要带上令牌）。

...
* （C）客户端使用上一步获得的授权，向认证服务器申请令牌（注：这一步可能是从客户端直接向认证服务器发送请求，也可能是从第三方应用程序服务器发送到认证服务器
，我们假想这一步认证服务器返回给客户端一个认证表单）。

* （D）认证服务器对客户端进行认证以后，确认无误，同意发放令牌(注:假设这一步是提交上一步认证服务器响应的认证表单,同时想一想，第三方应用程序怎么保存这个令牌的问题)。

* （E）客户端使用令牌，向资源服务器申请获取资源(注:这一步更可能是服务器对服务器的访问)。
